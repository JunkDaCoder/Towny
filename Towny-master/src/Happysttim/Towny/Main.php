		...
		switch($command->getName()){
			...
			if($sender->hasPermission("towny.command")){
				...
				switch($args[0]){
					...
					case "delete":
					if(!isset($args[1])){
						$sender->sendMessage(self::PREFIX.Color::RED." /t delete <village name>");
						return;
					}
					if(!$this->town->isTown($args[1])){
						$sender->sendMessage(self::PREFIX.Color::RED." Town does not exist!");
						return;
					}
					$this->town->removeTown($args[1]);
					$this->getServer()->broadcastMessage(self::PREFIX.Color::GOLD." {$sender->getName()} {$args[1]} has been deleted!");
					break;
					case "영역티켓":
					...
					case "reset":
					$this->town->reset();
					$this->getServer()->broadcastMessage(self::PREFIX.Color::RED." {$sender->getName()} Your village/town data has been reset!");
					break;
					case "자금":
